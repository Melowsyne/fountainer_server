...
# Checks beforehand everything the ESP32 needs to connect (PKI, .env,
# host IP SERVER_IP) and waits for the device connection at the end.
set -euo pipefail
cd "$(dirname "$(readlink -f "$0")")"

CA_DIR="../DO_NOT_COMMIT/CA"
DEVICE_EXPECTED_IP="SERVER_IP"   # SERVER_HOST in the ESP32 firmware + SAN in the server certificate
...
for f in server/server.crt.pem server/server.key.pem root/certs/ca.crt.pem; do
    [ -f "$CA_DIR/$f" ] || fail "PKI incomplete: $CA_DIR/$f missing."
done
[ -f devices.json ] || fail "devices.json missing (device registry)."
grep -q '^TLS_KEY_PASSWORD=' .env 2>/dev/null \
    || fail ".env missing or contains no TLS_KEY_PASSWORD (passphrase of the server key)."
...
if ! ip -br addr show | grep -q "$DEVICE_EXPECTED_IP"; then
    ...
    echo "         to exactly this address and will NOT reach the server."
    echo "         Remedy: sudo ip addr add $DEVICE_EXPECTED_IP/24 dev enp0s3"
fi

# ---------- (Re)start the stack --------------------------------------------
if [ -n "$(docker compose ps -q 2>/dev/null)" ]; then
    info "Stack already running — restarting."
fi
...
for i in $(seq 1 75); do
    if docker logs fountain_server 2>&1 | grep -q 'session proof valid'; then
        ...
        info "Done — stack fully running."
        exit 0
    fi
    sleep 1
done
echo "WARNING: server is running, but the Fountainer has not connected after"
echo "         75 s. Device powered on and on WLAN? Follow the log with:"
