...
/api/history delivers backfill via since_i that the old t filter would
hide."""
import asyncio
import tempfile
import time
    ...
    with tempfile.TemporaryDirectory() as fwdir, \
        ...
        async with app.serve():
            ...
            try:
                ...
                # Direct signed read: correlated history_batch.
                ...
                # Poller (immediately after connect): samples land in the chart
                ...
                # 90 s gap rule) — even if backfill samples with an
