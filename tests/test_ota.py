...
async def _held_device_gets_no_offer():
    """ota_hold set -> ota_none although a newer image exists."""
    ws_port = free_port()
    reg = DeviceRegistry(
        {DID: Device(DID, SERIAL, TOKEN, {"1": bytes.fromhex(KEY_HEX)}, ota_hold=True)})
    with tempfile.TemporaryDirectory() as fwdir:
        with open(os.path.join(fwdir, "fountain-9.9.9.bin"), "wb") as f:
            f.write(b"newer-but-held")
        app = FountainAppServer(reg, fwdir, ws_host="127.0.0.1",
                                ws_port=ws_port, http_host="127.0.0.1", http_port=0,
                                web_enabled=False)
        async with app.serve():
            sim = SimulatedDevice(f"ws://127.0.0.1:{ws_port}/ws", device_id=DID,
...
def test_ota_hold_answers_ota_none():
    asyncio.run(_held_device_gets_no_offer())


def test_registry_parses_ota_hold(tmp_path):
    path = tmp_path / "devices.json"
    path.write_text(json.dumps({
        "held":  {"serial": "1", "bearer_token": "t", "auth_keys": {}, "ota_hold": True},
        "free":  {"serial": "2", "bearer_token": "t", "auth_keys": {}},
    }))
    reg = DeviceRegistry.from_json(str(path))
    assert reg.get("held").ota_hold is True
    assert reg.get("free").ota_hold is False
