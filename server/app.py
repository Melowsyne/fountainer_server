...
class FountainAppServer:
    ...
    async def _on_connect(self, session, _msg) -> None:
        ...
        async def _fetch_config() -> None:
            try:
                ...
                # %r instead of %s: an empty TimeoutError yielded "failed: " without
                # any diagnosis — now at least the exception type is there.
    ...
    async def _poll_logs(self, session) -> None:
        ...
        while st.get("online"):
            # Do NOT disturb an active OTA download: the device's second TLS
            # session needs heap/CPU (incident 31.08.: pump OTA failed at 0%
            # while the poller was pulling in fast mode under sustained
            # overflow load) -> pause until the download is over.
            if (st.get("ota_status") or {}).get("state") == "downloading":
                await asyncio.sleep(2.0)
                continue
            try:
            ...
            if res.get("overflow"):
                # Additionally into the server log (the UI event feed alone went
                # unnoticed during the gap analysis of 23.08.) and ADAPTIVELY
                # into fast mode for 2 min: the device is currently producing
                # faster than the 60 s grid can collect.
    ...
    def _hist_add(self, device_id: str, t_ms: int, pressure_bar, relay,
        ...
        lands BEFORE already delivered samples), with a monotonic insertion ID i —
        the /api/history client polls via since_i and thus also receives
        older samples added later."""
        st = self._state(device_id)
    ...
    async def _poll_history(self, session) -> None:
        """Read back the device's 1 Hz pressure history: immediately after connect
        (this closes the offline gap), then every history_poll_s. The cursor lives
        in the device state and survives reconnects; a boot_id change resets it
        (RAM ring restarts at seq 1)."""
        device_id = session.device_id
        st = self._state(device_id)
        ...
        while st.get("online"):
            ...
            if boot_id != st.get("hist_boot") or next_seq <= int(st.get("hist_seq", 0)):
                st["hist_boot"], st["hist_seq"] = boot_id, 0   # reboot: ring restarted
            first_avail = int(res.get("first_seq_available") or 1)
            cursor = int(st.get("hist_seq", 0))
            if cursor and first_avail > cursor + 1:
                # Ring has evicted older samples — the gap is final
                # (only the last ~100 s can be backfilled). Never invent values.
                self.event(device_id,
                           f"pressure history: gap seq {cursor + 1}.."
                           f"{first_avail - 1} (ring overwrote)", "warn")
            # Wall-clock anchor: now_ms is the device uptime when the batch was built;
            ...
            for row in samples:
                ...
                # Plot all samples — including sensor faults (fallback value/0 bar),
                # so every device shows the same continuous 1 Hz curve as
                # the production device; the fault status remains visible in the
                # sample status or the device log. Relay is not contained in the
    ...
    async def _on_ota_check(self, session, msg) -> None:
        ...
        if dev is not None and dev.ota_hold:
            # Device-specific hold (devices.json "ota_hold": true): no
            # offer, no matter what lies in FIRMWARE_UPDATES. The inventory thus
            # stays global, the distribution is released per device.
            LOG.info("ota_check %s: OTA locked (ota_hold), currently on %s — no offer",
                     device_id, current)
            self.event(device_id, f"ota_check: held (ota_hold), currently on {current}")
        ...
        LOG.info("ota_check %s: offering %s -> %s (%d B, sha256=%s…)",
