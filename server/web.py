...
class AdminWeb:
    ...
    async def api_devices(self, request):
        ...
        for did in self.app.registry:
            ...
            out.append({
                ...
                "ota_hold": self.app.registry.get(did).ota_hold,
    ...
    async def api_history(self, request):
        ...
        ?since_i=<insertion id>: also delivers BACKFILLED older samples
    ...
    async def api_dp_write(self, request):
        ...
        if not isinstance(dp, dict) or not dp:
            return web.json_response({"error": "empty dp object"}, status=400)
    ...
    async def start(self):
        ...
        LOG.info("Admin UI on http://%s:%d (login: %s)", self.host, self.port, self.username)
