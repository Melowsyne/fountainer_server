...
Fountainer device (firmware_server.md; implementation plan AP7).
...
- Port 4443, NO bearer header, no device_id query (identity via mTLS
  + the device's hello).
- CA pinning, but NO hostname verification (device IP is DHCP-
  dynamic; SAN carries only the optional mDNS name <device_id>.local).
"""
import argparse
import asyncio
import json
